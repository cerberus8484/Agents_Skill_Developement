...
- For schema compilers, distinguish who controls the schema from who controls its values. An unknown-type fallback may be an internal robustness gap or a security risk; a direct-function probe alone does not establish external schema control.
- Before claiming a validation bypass, inspect callers and earlier guards. Record blocked or unknown paths as counterevidence, leaving other unexamined paths explicitly unassessed.
